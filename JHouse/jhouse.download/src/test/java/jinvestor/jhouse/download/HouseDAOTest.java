...
public abstract class HouseDAOTest {
	...
	private House makeHouse() throws ParseException {
		Date lsd = DateUtils.parseDate("8/15/13", "dd/mm/yy");
		lsd = DateUtils.truncate(lsd, Calendar.DAY_OF_MONTH);
		return House.builder().acres(0.8f)
				.address("123 Rose Lane, CherryVille MN, 12345").baths(3.5f)
				.beds((byte) 4).latitude(825938).longitude(-285293)
				.lastSoldDate(lsd).soldPrice(529600).zpid(198523L).build();
	}
}
